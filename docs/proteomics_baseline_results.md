...
**Script:** [`experiments/Proteomics/proteomics_baseline.py`](../experiments/Proteomics/proteomics_baseline.py)
**Companion:** [`docs/rf_baseline_results.md`](./rf_baseline_results.md) (the fused GE+Mut_CNV+Proteomics baseline this run is compared against)
**Date:** 2026-08-17
...
`rf_baseline_results.md`) that proteomics is under-represented and
...
caveat as `rf_baseline_results.md` §3 about unpinned `scikit-learn` versions
in `requirements.txt` still applies).

## 4. What this means for MCS16

Per `rf_baseline_results.md` §4.3, the RF baseline (fused or single-omics) is
...
1. Run the equivalent single-omics ablation for GE (transcriptomics) — the
   ...
   `experiments/Transcriptomics & Genomics/TransGen_Baseline.py`; a
...
3. Log this number in the running `docs/results.md` comparison table proposed
   in `rf_baseline_results.md` §5.3, once that table exists.
