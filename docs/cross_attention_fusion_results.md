...
**Script:** [`experiments/Cross Attention Fusion/cross_attention_baseline.py`](../experiments/Cross%20Attention%20Fusion/cross_attention_baseline.py)
**Companions:** [`docs/rf_baseline_results.md`](./rf_baseline_results.md), [`docs/proteomics_baseline_results.md`](./proteomics_baseline_results.md)
**Date:** 2026-08-17
...
1. **Learned fusion vs. flat concatenation** — this is the effect the module
   was built to test (per `docs/rf_baseline_results.md` §4.3's RF → MLP →
...
in training) — a ~3% gap, consistent with `rf_baseline_results.md`'s
...
- **Same drug-identity limitation as every other baseline so far** — one-hot
  drug encoding (§4.3 of `rf_baseline_results.md` already documents this):
...
3. **Log this number** in the running comparison table proposed in
   `rf_baseline_results.md` §5.3 once it exists (mean-only / RF-fused /
